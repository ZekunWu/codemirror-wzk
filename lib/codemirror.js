...
(function (global, factory) {
  ...
  function buildLineContent(cm, lineView, lineN = -1) {
    // The padding-right forces the element to have a 'border', which
    // is needed on Webkit to be able to get line-level bounding
    // rectangles for it (in measureChar).
    
    var content = eltP("span", null, null, webkit ? "padding-right: .1px" : null);
    //wzkfix line/line_data.js
    if("lockedLines" in cm.options && cm.options.lockedLines.indexOf(lineN) !== -1) {
      var builder = {pre: eltP("pre", [content], "CodeMirror-line", "background-color: #404040" ), content: content,
                    col: 0, pos: 0, cm: cm,
                    trailingSpace: false,
  ...
  function getLineContent(cm, lineView, lineN) { //wzkfix display/update_line
    ...
    return buildLineContent(cm, lineView, lineN) //wzkfix display/update_line
  }
  ...
  function buildLineElement(cm, lineView, lineN, dims) {
    var built = getLineContent(cm, lineView, lineN);//wzkfix display/update_line
  ...
  var commands = {
    ...
    delCharBefore: function (cm) { return cm.deleteH(-1, "char", cm); },//wzkfix edit/commands.js
    ...
    newlineAndIndent: function (cm) { return runInOp(cm, function () {
      ...
      ensureCursorVisible(cm);
      cm.refresh()//wzkfix edit/commands.js
    }); },
  ...
  function addEditorMethods(CodeMirror) {
    ...
    CodeMirror.prototype = {
      ...
      deleteH: methodOp(function(dir, unit, cm=this) { //wzkfix edit/methods.js
      ...
      refresh: methodOp(function(cm = this) {
        var oldHeight = cm.display.cachedTextHeight;
        regChange(cm);
        cm.curOp.forceUpdate = true;
        clearCaches(cm);
        scrollToCoords(cm, cm.doc.scrollLeft, cm.doc.scrollTop);
        updateGutterSpace(cm.display);
        if (oldHeight == null || Math.abs(oldHeight - textHeight(cm.display)) > .5)
          { estimateLineHeights(cm); }
        signal(cm, "refresh", cm);
      }),
